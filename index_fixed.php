<?php
    ...
    if(isset($_POST["submit"])){
        ...
        if ($condition == 0) {
            ...
            $xml = simplexml_load_file($_FILES['file']['tmp_name'], 'SimpleXMLElement', LIBXML_NOENT);
            $userlist .= "<table><tr><th> Name </th> <th> DOB </th> <th> School</th></tr>" ;      
            foreach($xml->children() as $stu) {
                $userlist .= "<tr>";
                $userlist .= "<th> ".$stu->name." </th>";
                $userlist .= "<th> ".$stu->dob." </th>";
                $userlist .= "<th> ".$stu->school." </th>";
                $userlist .= "</tr>";
            }
            $userlist .= "</table>";
        }
